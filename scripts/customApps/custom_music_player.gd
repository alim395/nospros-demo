...
func _ready() -> void:
	...
	if secretSongs:
		...
		for s in secretSongs:
			...
			if secretSongs.get(s) in globalParameters.secret3Array:
				secretFlags.set(s,true)
	else:
...
func _on_seek_back_pressed() -> void:
	isSeeking = true
	_on_pause_button_pressed()
	seekSlider.value -= 10
	MusicManager.current_player.seek(seekSlider.value)
...
func closeWindow() -> void:
	...
	if get_parent():
		get_parent().get_node("%Taskbar").closeTask("Music")
	queue_free()
...
func checkS3Songs(songName : String) -> void:
	for s in secretSongs:
		if secretSongs.get(s) == songName:
			...
			if not(songName in globalParameters.secret3Array):
				print("Secret not found yet")
				globalParameters.secret3Array.append(songName)
				print(globalParameters.secret3Array)
			return
	radialVisualizer.clearSecret()
...
func updateTitle() -> void:
	if currentTrack:
		setTitle(currentTrack.name + " - Music Player")
	else:
		setTitle("Unknown - Music Player")

func updateProgramIcon() -> void:
	setProgamIcon(globalParameters.icon_dict.get("Music"))
